...
| 6 — Lint debt cleanup | ✅ cerrada | Resueltas las 62+ violations heredadas. Reglas endurecidas a `error` en `biome.json` (useTemplate, noForEach, noArrayIndexKey, useExhaustiveDependencies, useButtonType y +8 más). Algunas pocas excepciones documentadas con `biome-ignore` (mulberry32 closure-mutation, HarmonicWheel custom radio, Preview drag-drop). 331/331 tests verde. Detalle en `TASKS.md` Fase 6. |
