...
- `_lib/orders-store.ts` — `KVBackend` interface (`get`/`set`) with two implementations: `netlifyBlobsBackend()` for prod, `inMemoryBackend()` for tests. `consumeDownload` does read-check-write; not transactional, but acceptable for the 5-allowance budget. Order record = `{orderId, lsOrderId, packSlug, email, downloadsRemaining, expiresAt, createdAt}`. **Schema note** (changed pre-launch, see ADR 0002): `orderId` is our own UUID (primary key, travels in the JWT, opaque to buyers) and `lsOrderId` is the buyer-visible id Lemon Squeezy assigns. Keys are namespaced as `order:{orderId}` and `ls:{lsOrderId} → { orderId }` (secondary pointer used by `/recover`, which only ever sees the LS id). The change is destructive vs. the previous shape — wipe the local Blobs store after pulling.
